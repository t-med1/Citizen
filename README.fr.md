...
Conçue comme un projet de référence complet, en couches, de forme
« production » : entités → repositories → services (interfaces +
implémentations) → DTOs/mappers → contrôleurs Spring MVC → interface
Thymeleaf/Bootstrap, sécurisée avec Spring Security et un contrôle d'accès
basé sur les rôles.
